...
$rows = $stmt->fetchAll();
?>
<div id="achievements-list" class="flex flex-col gap-4 mb-6">
<?php if(empty($rows)): ?>
<p class="text-on-surface-variant font-body-md text-center py-8 border-2 border-dashed border-outline-variant rounded-xl">No achievements yet. Add one below.</p>
...
<div class="bg-surface border border-outline-variant/40 rounded-xl p-5 flex justify-between items-start gap-4">
  <div>
    ...
    <p class="text-label-sm text-on-surface-variant"><?= htmlspecialchars($r['issuer']) ?> | <?= htmlspecialchars($r['date']) ?></p>
    <?php if($r['description']): ?><p class="text-sm text-on-surface-variant mt-2"><?= htmlspecialchars($r['description']) ?></p><?php endif; ?>
  </div>
  <form method="POST" action="?page=profile&step=6">
...
<div class="bg-surface-container-low rounded-2xl p-6 border border-outline-variant/30">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div>
      ...
      <input name="title" placeholder="Best Innovator Award" class="w-full border border-outline-variant rounded-xl px-4 py-3 bg-surface font-body-md focus:outline-none focus:ring-2 focus:ring-primary/40">
    </div>
    <div>
      <label class="block font-label-md text-on-surface-variant mb-1">Issuer / Organisation</label>
      <input name="issuer" placeholder="Google, College..." class="w-full border border-outline-variant rounded-xl px-4 py-3 bg-surface font-body-md focus:outline-none focus:ring-2 focus:ring-primary/40">
    </div>
    <div>
      <label class="block font-label-md text-on-surface-variant mb-1">Date</label>
      <input name="date" type="month" class="w-full border border-outline-variant rounded-xl px-4 py-3 bg-surface font-body-md focus:outline-none focus:ring-2 focus:ring-primary/40">
    </div>
    <div class="md:col-span-2">
      <label class="block font-label-md text-on-surface-variant mb-1">Description (optional)</label>
      <textarea name="description" rows="2" placeholder="Brief about this achievement..." class="w-full border border-outline-variant rounded-xl px-4 py-3 bg-surface font-body-md focus:outline-none focus:ring-2 focus:ring-primary/40 resize-none"></textarea>
    </div>
  </div>
